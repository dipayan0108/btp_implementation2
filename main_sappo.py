...
class PPOAgent(tf.keras.Model):
    ...
    def __init__(self, name="PPOAgent", **kwargs):
        ...
        self.prev_mean = None              # stores previous policy mean (for KL)
        self.kl_threshold = 0.05           # detection sensitivity
        self.entropy_min = 0.2             # entropy floor (detect overconfidence)
        self.epsilon = 0.05                # perturbation magnitude |δ|∞≤ε
        self.kappa = 0.1                   # regularization weight
    ...
    def call(self, obs, train):
        ...
        #new implementation for kl and entropy
        std = tf.exp(self.log_std)
        dist = tfd.MultivariateNormalDiag(loc=mean, scale_diag=std)
        entropy = tf.reduce_mean(dist.entropy())

        if self.prev_mean is not None:
            prev_dist = tfd.MultivariateNormalDiag(loc=self.prev_mean, scale_diag=std)
            kl_div = tf.reduce_mean(tfd.kl_divergence(prev_dist, dist))

            if kl_div > self.kl_threshold:
                print(f"🚨 Attack suspected — KL={kl_div.numpy():.4f}")
            if entropy < self.entropy_min:
                print(f"⚠️ Entropy drop — possible biased observation")
        self.prev_mean = mean
    ...
    def learn(self):
        ...
        for i in range(self.n_updates_per_iteration):
            with tf.GradientTape() as tape_a, tf.GradientTape() as tape_c:
                ...
                # --- SA-PPO Regularizer (Eq. 5) ---
                eps = self.epsilon
                perturbed_states = old_states + tf.random.uniform(
                shape=old_states.shape, minval=-eps, maxval=eps)

                mu_clean = self.actor(old_states)
                mu_pert = self.actor(perturbed_states)

                # R_PPO = ½ E[(μ(ŝ)−μ(s))ᵀΣ⁻¹(μ(ŝ)−μ(s))]
                reg_loss = 0.5 * tf.reduce_mean(tf.square((mu_clean - mu_pert) / tf.exp(self.log_std)))

                # L_SA-PPO = L_PPO + κ·R_PPO
                actor_loss += self.kappa * reg_loss
